...
public class DataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        // 1. ALWAYS ENSURE SYSTEM ADMIN EXISTS
        seedInitialAdmin();

        // 2. ALWAYS ENSURE DEMO TEST ACCOUNTS EXIST FOR IMMEDIATE LOGIN
        seedDemoStudentAndInvestorData();

        // 3. ENSURE ALL FUNDING REQUESTS HAVE AI RISK SCORES COMPUTED
    ...
    private void seedInitialAdmin() {
        if (!userRepository.existsByEmail(initialAdminEmail)) {
            ...
            log.info("System Admin account created: {}", initialAdminEmail);
        }
    }

    private void seedDemoStudentAndInvestorData() {
        // Seed Student 1 (Aarav Deshmukh - COEP Pune CS)
        if (!userRepository.existsByEmail("student@skillbond.com")) {
            ...
            log.info("Demo Student 1 created: student@skillbond.com");
        }

        // Seed Student 2 (Ananya Patil - VJTI Mumbai IT)
        if (!userRepository.existsByEmail("ananya.patil@skillbond.com")) {
            ...
            log.info("Demo Student 2 created: ananya.patil@skillbond.com");
        }

        // Seed Investor User
        if (!userRepository.existsByEmail("investor@skillbond.com")) {
            ...
            log.info("Demo Investor created: investor@skillbond.com");
        }
    }
}
